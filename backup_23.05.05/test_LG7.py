...
#### To avoid an error like this happening again in the future, implement lines of code that test to see if the code is being run locally or on one o fthe 2 servers, then have variables rpedefine file locations based on where things are on those servers. This will help avoid issues where locations and variables are different between locations, so taht we dont have to rename everything in the code
...
sp.run(shlex.split(f"gatk --java-options '-Xmx450G' GenomicsDBImport --genomicsdb-workspace-path {'/Data/mcgrath-lab/Data/CichlidSequencingData/Databases/' + lg7 + '_database'} --intervals lg7.interval_list --sample-name-map sample_map_utaka_lg7.txt --interval-merging-rule OVERLAPPING_ONLY --max-num-intervals-to-import-in-parallel 4 --overwrite-existing-genomicsdb-workspace"))
