...
def main():
    ...
    try:
        ...
        # Open both BAMs once for the whole sample. fetch_reads_near_insertion
        # accepts open handles, and opening a 10 GB BAM plus its index 4785
        # times is pure waste.
        main_bam = pysam.AlignmentFile(fm_obj.localBamFile, "rb")
        disc_bam = None
        if os.path.exists(fm_obj.localDiscordantBamFile):
            # An unindexed discordant BAM forces a full scan per variant, which
            # is the difference between minutes and hours at this variant count.
            # Build the index rather than warning about it: indexing a discordant
            # BAM takes seconds and the file is local and disposable anyway.
            disc_bam = pysam.AlignmentFile(fm_obj.localDiscordantBamFile, "rb")
            if not disc_bam.has_index():
                disc_bam.close()
                log(f"{args.SampleID}: discordant BAM has no index, building one")
                try:
                    pysam.index(fm_obj.localDiscordantBamFile)
                except Exception as e:
                    man.add_warning(
                        f"could not index the discordant BAM ({e}); falling back "
                        f"to a full scan per variant, which will be very slow at "
                        f"{expected_lv} large variants")
                disc_bam = pysam.AlignmentFile(fm_obj.localDiscordantBamFile, "rb")
                if disc_bam.has_index():
                    log(f"{args.SampleID}: discordant BAM indexed")

        # Coordinate-sorted input is required for indexing to work at all.
        if disc_bam is not None and not disc_bam.has_index():
            man.add_warning(
                "discordant BAM remains unindexed -- it may not be coordinate "
                "sorted. `samtools sort` then `samtools index` would fix it.")
        ...
        for i, row in lv_dt.iterrows():
            ...
            reads = classifier.fetch_reads_near_insertion(
                bam=main_bam,
                discordant_bam=disc_bam,
                # Query the discordant BAM by region instead of scanning it
                # whole. Without this, every variant re-reads the entire file:
                # 4785 variants meant 4785 full passes, which was 7 of the 7.2
                # hours this stage took on the first sample.
                disc_region_fetch=True,
            )
