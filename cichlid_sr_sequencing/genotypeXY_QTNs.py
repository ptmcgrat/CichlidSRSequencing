...
def already_done(out_vcf, sampleID):
    ...
    # n_missing_total is the true count; missing_sites may be truncated by
    # --max-missing-report, so counting the list would undercount badly.
    n_missing = m.get("n_missing_total") or len(m.get("missing_sites", []))
    accounted = m.get("observed_out", 0) + n_missing
...
def check_manifest(out_vcf, sampleID, max_missing=0):
    ...
    # The unexplained list is drawn from the truncated detail, so scale it back up
    # to the full missing count before comparing against the threshold.
    detail = len(m.get("missing_sites", []))
    if detail and n_missing > detail:
        n_unexplained = int(round(n_unexplained * n_missing / detail))
