...
def main():
    ...
    # Packed genotypes: 2 bits per call, so the per-variant sample distribution
    # can be shown without shipping a byte-per-field matrix.
    gt_b64, gt_meta = None, None
    gt_bin = web + f"{args.contig}_genotypes.bin"
    gt_json = web + f"{args.contig}_genotypes_samples.json"
    for pth in (gt_bin, gt_json):
        if not os.path.exists(pth):
            try:
                fm_obj.downloadData(pth)
            except Exception:
                pass
    if os.path.exists(gt_bin) and os.path.exists(gt_json):
        gt_meta = json.load(open(gt_json))
        raw = open(gt_bin, "rb").read()
        expect = (gt_meta["n_variants"] * gt_meta["n_samples"] + 3) // 4
        if len(raw) != expect:
            warn(f"genotype matrix is {len(raw)} bytes, expected {expect}; "
                 f"skipping the sample-distribution view")
            gt_meta = None
        else:
            gt_b64 = base64.b64encode(raw).decode("ascii")
            log(f"genotypes: {gt_meta['n_variants']:,} x "
                f"{gt_meta['n_samples']} ({len(raw)/1e6:.1f} MB packed)")
    else:
        warn("no packed genotypes found; re-run buildVariantStats.py to enable "
             "the per-variant sample distribution")
    ...
    payload = {
        ...
        "gt_b64": gt_b64,
        "gt_samples": (gt_meta or {}).get("samples"),
        "gt_n_samples": (gt_meta or {}).get("n_samples", 0),
    }
