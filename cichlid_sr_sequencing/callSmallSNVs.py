...
parser.add_argument('-p', '--ProjectIDs', type = str, help = 'Restrict analysis to a specific ProjectID')
...
for sample in good_samples:
	...
	for read in align_file.fetch(until_eof=True):
		...
		if read.cigarstring is not None:
			if read.has_tag('XM') or read.is_duplicate: # Adapters present or read is a duplicate - ignore
				continue
			for pair in read.cigartuples:
				if pair[0] == 4 and pair[1] > 4:
					...
					break
		# Chimeric
		if read.has_tag('SA') and not read.is_duplicate:
			chimeric.write(read)
