...
class FileManager():
	...
	def setSamples(self, projectIDs = None, sampleIDs = None, ecogroupIDs = None, analysisType = None):
		...
		elif analysisType is not None:
			bad_analyses = []
			analysisType = analysisType[0].split(',')
			for analysis in analysisType:
				if analysis not in s_dt.columns.to_list():
					bad_analyses.add(analysis)
