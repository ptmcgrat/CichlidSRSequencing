...
# for now I will hard_code in that all filenames will have the prefix "612_498_cohort" since that's the cohort I will run this on the first time I use the script
# NOTE: need to update script to allow for different prefix names

# stats for mean depth per sample (column)
ind_depth <- read_delim("498_cohort.idepth", delim = "\t",
...
var_depth <- read_delim("498_cohort.ldepth.mean", delim = "\t",
...
var_site_depth <- read_delim("498_cohort.ldepth", delim = "\t",
...
ind_miss  <- read_delim("498_cohort.imiss", delim = "\t",
...
var_miss <- read_delim("498_cohort.lmiss", delim = "\t",
...
var_freq <- read_delim("498_cohort.frq", delim = "\t",
...
ind_het <- read_delim("498_cohort.het", delim = "\t",
...
var_qual <- read_delim("498_cohort.lqual", delim = "\t",
