...
for contig in contigs:
    p = sp.Popen(shlex.split(f"gatk GenomicsDBImport --genomicsdb-workspace-path {'/home/ad.gatech.edu/bio-mcgrath-dropbox/Data/CichlidSequencingData/Databases' + contig + '_database'} --intervals {contig} --sample-name-map sample_map.txt --reader-threads 4"))
    processes.append(p)
...
for contig in contigs:
	p2 = sp.Popen(shlex.split(f"gatk GenotypeGVCFs -R /home/ad.gatech.edu/bio-mcgrath-dropbox/Data/CichlidSequencingData/Genomes/Mzebra_UMD2a/GCF_000238955.4_M_zebra_UMD2a_genomic.fna -V {'gendb://../../Data/CichlidSequencingData/Databases/' + contig + '_database'}  -O {'/home/ad.gatech.edu/bio-mcgrath-dropbox/Data/CichlidSequencingData/Outputs/' + contig + '_output.vcf'}"))
